...
fn magic(path: &Path) -> anyhow::Result<Vec<u8>> {
...
pub trait Substituter: Sized {
    /// If this type can handle the scheme of this url,
    /// returns an instance, otherwise None
    /// cache_dir is a directory where requested debuginfo must be stored
    ...
    fn fetch(&self, path: &Path) -> anyhow::Result<Option<PathBuf>>;
    ...
    fn fetch_debuginfo_from(
        ...
        let file = self
            .fetch(path)
            .with_context(|| format!("fetching {} from {}", path.display(), self.url()))?;
        let file = match file {
            None => return Ok(None),
            Some(f) => f,
        };
        let tempfile;
        let target;
        let dir_to_add = match &magic(file.as_path())? {
            m if m.starts_with(ELF_MAGIC) => {
                ...
                std::fs::copy(file.as_path(), parent.as_path())
            ...
            m if m.starts_with(b"{") => {
                ...
                // this is a json redirect
                let file = std::fs::File::open(file.as_path()).with_context(|| {
                    format!("opening {} to deserialize as json", path.display())
                })?;
                ...
                return self.fetch_debuginfo_from(redirect_path.as_path(), max_redirects - 1);
            }
            m => {
                let nar_file = if m.starts_with(NAR_MAGIC) {
                    ...
                    tempfile = tempfile::NamedTempFile::new().context("tempfile")?;
                    let fd = std::fs::File::open(&file).context("unxz")?;
                    compress_tools::uncompress_data(fd, &tempfile).with_context(|| {
                        format!("unpacking {} from {}", file.display(), self.url())
                    })?;
                    if magic(tempfile.path())
                ...
                // unpack the nar
                let fd = std::fs::File::open(nar_file)?;
        ...
        cmd.arg("--add");
        cmd.arg(dir_to_add);
        let output = cmd.output().context("nix-store --add")?;
        anyhow::ensure!(
...
#[test]
fn file_substituter_from_url() {
    let d = TempDir::new().unwrap();
    assert!(matches!(
        FileSubstituter::from_url("https://cache.nixos.rg"),
        Ok(None)
    ));
    assert!(matches!(
...
fn file_substituter_fetch() {
    ...
    assert_eq!(ok.fetch(Path::new("./file")).unwrap().unwrap(), path);
}
